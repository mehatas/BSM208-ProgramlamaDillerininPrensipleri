...
short * splitDigits(int val){//javadaki private benzetimi (imzasıbaslik dosyasina yazilmaz).
	int digitlength = floor(log10(abs(val)))+1;
	short *digits =(short*)malloc(sizeof(short)*digitlength);
	int index ;
	for(index=digitlength-1;index>=0;index--){
		digits[index]=val%10;
		val/=10;
	}
	return digits;
